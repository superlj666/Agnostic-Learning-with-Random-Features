function [bestc, bestg, bestcv] = automaticParameterSelectionlambda(trainLabel, trainData, Ncv, option)
...
% % %             cv = get_cv_ac(trainLabel, [(1:NTrain)' trainData*trainData'], cmd, Ncv);
           ...
           cmd = log2c;
            cv = get_cv_aclambda(trainLabel, trainData, cmd, Ncv);
            if (cv >= bestcv),
                bestcv = cv; bestLog2c = log2c; bestLog2g = log2g;
                bestc = bestLog2c; bestg = bestLog2g;
            end
...
disp(['The best parameters, yielding Accuracy=',num2str(bestcv*100),'%, are: lambda=',num2str(bestc)]);
